...
single-pass probe under-reported by 10--15\%, which inflated the reported
fraction of roofline from 54\% to 62\%.
...
Fraction of measured roofline & $\sim$57\% \\
\bottomrule
\end{tabular}
...
An instrumented decode contradicts the byte analysis of
Table~\ref{tab:bytes}. The head is 42\% of wall time despite being 72\% of the
bytes, because the two halves run at opposite ends of the machine's efficiency
range: 48.8 GB/s for the head against 13.9 GB/s for the ternary path.
...
\bottomrule
\end{tabular}
\caption{Where decode time is spent, 4 threads.}
\label{tab:time}
\end{table}

The head is already at roughly 92\% of this machine's measured ceiling, and no
kernel work will move it: forcing the dense kernel to scalar, AVX2 and AVX-512
in turn gives 28.8, 28.1 and 27.1 ms respectively. The ternary path is the
opposite. It is compute-bound in the kernel at approximately 15.8 GB/s per core,
barely above what it achieves from DRAM, which means it would scale with cores
if the runtime could use them.

\subsection{Why it does not scale}

A decode step issues 210 ternary matvecs whose largest is 4.4 MB. Each currently
pays a work-stealing fork/join. Lowering the threshold at which matvecs
parallelise makes matters dramatically worse, not better
(Table~\ref{tab:par}, row \emph{rayon-global}): even two threads is
...
Table~\ref{tab:par} rules out the cheap explanations. A dedicated pool sized to
the requested thread count helps substantially and is still worse than serial
execution. A broadcast primitive helps more and is still worse. Only a
persistent worker pool on an atomic spin barrier, the mechanism ggml uses, beats
serial execution, at 2.9$\times$ on the layer path. End to end, in a
back-to-back comparison, that prototype lifts scaling from 1.15$\times$ to
1.79$\times$ and decode by 1.59$\times$ at eight threads. It is a prototype: its
lifetime handling is not sound and it is not merged.
