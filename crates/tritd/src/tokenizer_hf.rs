...
impl HfTokenizer {
    pub fn from_file(path: &std::path::Path) -> Result<Self> {
        ...
        let eos = EOS_CANDIDATES.iter().filter_map(|n| inner.token_to_id(n)).collect();
        Ok(Self { inner, eos })
    }
}
...
impl Tokenizer for HfTokenizer {
    ...
    fn decode(&self, ids: &[u32], skip_special: bool) -> Result<String> {
        self.inner.decode(ids, skip_special).map_err(anyhow::Error::msg)
    }
    ...
    fn id_to_bytes(&self, id: u32) -> Result<Vec<u8>> {
        ...
        Ok(self.inner.decode(&[id], false).map_err(anyhow::Error::msg)?.into_bytes())
    }
