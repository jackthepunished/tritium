...
- **Integer MLP path (Phase 4):** the squared-ReLU stage never exists in f32. With `g = acc_g * S_g` and `u = acc_u * S_u` from the gate/up matvecs (uniform positive scales), `relu(g)^2 * u = t * K` for integer `t = relu(acc_g)^2 * acc_u` (`abs(t) < 2^55`, exact in i64) and uniform `K = S_g^2 * S_u` — and by norm folding the down projection's absmax codes come from `t .* gain` directly (quantized through the f64 code-search step). Hardware cost: two streaming integer multiplies per element (inherent to relu2; ~2 DSPs), outside the multiplier-free ternary weight datapath. Proved against the real checkpoint (identical metrics and greedy text vs the folded f32 path).
